      ...
      <div className="grid min-w-0 grid-cols-1 gap-[14px] xl:grid-cols-[minmax(450px,0.84fr)_minmax(520px,1.16fr)]">
        <section className="flex min-h-[550px] min-w-0 flex-col rounded-[20px] border border-[#eaecf0] bg-white/95 p-4 shadow-[0_1px_2px_rgba(16,24,40,0.04),0_8px_24px_rgba(16,24,40,0.035)]">
          ...
          <div className="border-b border-[#f1f3f7]">
            ...
            <label className="block min-w-0 border-t border-[#f1f3f7] py-3">
              <span className="mb-1.5 block text-[10px] text-[#667085]">{translate("studio.field.aspectRatio")}</span>
              <select value={ratio} disabled={ratioDisabled} onChange={(event) => onRatioChange(event.target.value)} className="min-h-10 w-full border-0 bg-transparent text-[13px] font-bold text-[#101828] outline-none disabled:opacity-50">
          ...
          <div className="mt-2.5 grid grid-cols-2 gap-y-3">
            ...
            <div className="flex min-w-0 items-end ps-3"><button type="button" aria-expanded={moreSettingsOpen} onClick={() => setMoreSettingsOpen((value) => !value)} className="min-h-11 w-full rounded-[10px] border border-[#eaecf0] bg-white px-2 text-xs font-semibold text-[#344054]">{"\u2637"} &nbsp; {translate(moreSettingsOpen ? "studio.workbench.hideAdvancedSettings" : "studio.workbench.advancedSettings")}</button></div>
          </div>

          {moreSettingsOpen ? <div className={`mt-3 grid gap-3 rounded-xl border border-[#eaecf0] bg-[#fcfcfe] p-3 ${showAudioControl ? "sm:grid-cols-2" : ""}`}>
            <label className="min-w-0 rounded-[10px] border border-[#eaecf0] bg-white px-3 py-2">
              <span className="mb-1 block text-[10px] text-[#667085]">{translate("studio.field.seed")}</span>
              <input value={seed} onChange={(event) => onSeedChange(event.target.value.replace(/[^\d]/g, "").slice(0, 12))} placeholder={translate("studio.placeholder.random")} inputMode="numeric" className="min-h-8 w-full border-0 bg-transparent text-[13px] font-bold text-[#101828] outline-none placeholder:text-[#98a2b3]" />
            </label>
        ...
        <section className="min-h-[550px] min-w-0 overflow-hidden rounded-[20px] border border-[#eaecf0] bg-white/95 p-3.5 shadow-[0_1px_2px_rgba(16,24,40,0.04),0_8px_24px_rgba(16,24,40,0.035)]">
          ...
          {promptShowcases.length > 1 ? <div className="mt-3 grid grid-cols-[44px_minmax(0,1fr)_44px] items-center gap-1.5 sm:gap-2.5"><button type="button" aria-label={translate("studio.workbench.previous")} onClick={() => setSelectedShowcaseIndex((selectedShowcaseIndex - 1 + promptShowcases.length) % promptShowcases.length)} className="h-11 w-11 rounded-full border border-[#eaecf0] bg-white text-[#475467]">{"\u2039"}</button><div className="grid min-w-0 grid-cols-3 gap-1.5 p-0.5 sm:flex sm:gap-2.5 sm:overflow-x-auto">{promptShowcases.map((showcase, index) => <button key={showcase.videoUrl} type="button" onClick={() => useShowcase(showcase, index)} className={`relative aspect-video min-w-0 w-full overflow-hidden rounded-[8px] border bg-[#f6f7fb] transition hover:-translate-y-px sm:min-w-[128px] sm:flex-1 sm:rounded-[10px] ${selectedShowcaseIndex === index ? "border-white shadow-[0_0_0_2px_#7a6cff]" : "border-[#eaecf0]"}`}><img src={showcase.posterUrl} alt="" className="h-full w-full object-cover" /><span className="absolute start-1 top-1 rounded-md bg-white/90 px-1 py-0.5 text-[8px] font-extrabold text-[#344054] sm:start-1.5 sm:top-1.5 sm:px-1.5 sm:py-1 sm:text-[9px]">{showcase.duration}</span></button>)}</div><button type="button" aria-label={translate("studio.workbench.next")} onClick={() => setSelectedShowcaseIndex((selectedShowcaseIndex + 1) % promptShowcases.length)} className="h-11 w-11 rounded-full border border-[#eaecf0] bg-white text-[#475467]">{"\u203A"}</button></div> : null}
        </section>
      </div>
