...
const INTENTS: Array<{ title: string; body: string; href: string; icon: string; color: string; tools: string[] }> = [
  ...
  { title: "studio.workspace.intent.avatar", body: "studio.workspace.intent.avatarBody", href: "/studio?mode=avatar&workflow=avatar-video&provider=minimax-h3-max-turbo-video", icon: "💬", color: "bg-[#fff1f6]", tools: ["studio.nav.avatar"] },
    ...
    <div className="w-full min-w-0 max-w-full pb-4 text-start">
      ...
      <div className="mt-4 grid min-w-0 gap-4 xl:grid-cols-[0.92fr_1.08fr]">
        <section className="relative flex min-w-0 flex-col justify-between overflow-hidden rounded-[20px] bg-[linear-gradient(135deg,#6955f6,#8a5df7)] p-5 text-white shadow-[0_12px_30px_rgba(106,90,249,0.2)] sm:p-6">
          ...
          <div className="relative"><p className="text-[10px] font-black uppercase tracking-[0.13em] text-white/65">{t("creditShop.eyebrow")}</p><h2 className="mt-2 text-xl font-black tracking-[-0.025em] sm:text-2xl">{t("creditShop.title")}</h2><p className="mt-2 max-w-xl text-sm leading-6 text-white/75">{t("creditShop.subtitle")}</p><div className="mt-4 grid grid-cols-2 gap-2">{["creditShop.oneTime", "creditShop.shared"].map((key) => <span key={key} className="rounded-xl border border-white/15 bg-white/10 px-2.5 py-2 text-center text-[10px] font-bold text-white/90">✓ {t(key)}</span>)}</div></div>
          <button type="button" onClick={onUpgrade} className="relative mt-5 min-h-11 w-full rounded-xl bg-white px-4 text-sm font-black text-[#6757f6] shadow-sm transition hover:-translate-y-px sm:w-auto sm:self-start">{t("creditShop.buy")}</button>
        </section>
